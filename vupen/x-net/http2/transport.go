...
func (cs *clientStream) frameScratchBufferLen(maxFrameSize int) int {
	// Vupen: upstream caps this scratch buffer at 512 KiB, sized by the PEER's
	// SETTINGS_MAX_FRAME_SIZE. A streaming request body (XHTTP stream-one: one
	// POST per tunnelled TCP connection, unknown length) holds it for the whole
	// life of the stream, so a server advertising large frames pins 512 KiB per
	// live connection: 74 streams = 37 MB inside a ~50 MB Network Extension
